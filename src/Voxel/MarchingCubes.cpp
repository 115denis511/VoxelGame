...
inline void engine::MarchingCubes::addTriangle(const glm::vec3 &v0, const glm::vec3 &v1, const glm::vec3 &v2, MarchingCubesCase& currentCase) {
    ...
    for (int vX = 0; vX < 8; vX++) {
        glm::vec4 v0vertexSized(v0.x + POSITION_OFFSET + OFFSETS_STRENGTH[vX] * directionMul[v0data.direction].x, 
                                v0.y + POSITION_OFFSET + OFFSETS_STRENGTH[vX] * directionMul[v0data.direction].y, 
                                v0.z + POSITION_OFFSET + OFFSETS_STRENGTH[vX] * directionMul[v0data.direction].z, 0.f);

        for (int vY = 0; vY < 8; vY++) {
            glm::vec4 v1vertexSized(v1.x + POSITION_OFFSET + OFFSETS_STRENGTH[vY] * directionMul[v1data.direction].x, 
                                    v1.y + POSITION_OFFSET + OFFSETS_STRENGTH[vY] * directionMul[v1data.direction].y, 
                                    v1.z + POSITION_OFFSET + OFFSETS_STRENGTH[vY] * directionMul[v1data.direction].z, 0.f);

            for (int vZ = 0; vZ < 8; vZ++) {
                glm::vec4 v2vertexSized(v2.x + POSITION_OFFSET + OFFSETS_STRENGTH[vZ] * directionMul[v2data.direction].x, 
                                        v2.y + POSITION_OFFSET + OFFSETS_STRENGTH[vZ] * directionMul[v2data.direction].y, 
                                        v2.z + POSITION_OFFSET + OFFSETS_STRENGTH[vZ] * directionMul[v2data.direction].z, 0.f);
